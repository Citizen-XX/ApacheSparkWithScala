...
object MaxTemperaturesDataset {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val spark = SparkSession.builder.appName("MaxTemperaturesDataset").master("local[*]").getOrCreate()
    ...
    val maxTempsByStationF = maxTempsByStation.withColumn("temperature",round($"max(temperature)"*0.1f*(9.0f/5.0f)+32.0f,2)).select("stationID","temperature").sort("temperature")
